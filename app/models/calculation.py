...
class Power(Calculation):
    """
    Power (exponentiation) calculation subclass.

    Implements sequential, left-associative exponentiation starting from
    the first number.
    ...
    def get_result(self) -> float:
        """
        Compute the result of repeated exponentiation.

        Returns:
            float: The result of (((inputs[0] ** inputs[1]) ** inputs[2]) ** ...)

        Raises:
            ValueError: If inputs are not a list or fewer than 2 numbers were
                provided, or if the operation produces a complex / undefined
        ...
        for value in self.inputs[1:]:
            ...
            try:
                ...
                raise ValueError(f"Power operation overflowed: {exc}")
            # Reject complex results (e.g., (-1) ** 0.5)
            if isinstance(result, complex):
                raise ValueError("Power result is not a real number.")
        return float(result)


class Modulus(Calculation):
    """
    Modulus (remainder) calculation subclass.

    Implements sequential, left-associative modulus starting from the first
    number.
    ...
    def get_result(self) -> float:
        """
        Compute the result of repeated modulus.

        Returns:
            float: The result of (((inputs[0] % inputs[1]) % inputs[2]) % ...)

        Raises:
            ValueError: If inputs are not a list, fewer than 2 numbers were
                provided, or any divisor is zero.
        """
        if not isinstance(self.inputs, list):
            raise ValueError("Inputs must be a list of numbers.")
        if len(self.inputs) < 2:
...
class SquareRoot(Calculation):
    ...
    Unlike the other operations, square root is unary: it takes a single
    non-negative input and returns its principal square root.
    ...
    def get_result(self) -> float:
        """
        Compute the principal square root of the single input value.

        Returns:
            float: The non-negative square root of inputs[0].

        Raises:
            ValueError: If inputs is not a list, does not contain exactly one
                number, or contains a negative number.
        """
        if not isinstance(self.inputs, list):
            raise ValueError("Inputs must be a list of numbers.")
        if len(self.inputs) != 1:
            raise ValueError("Square root requires exactly one input.")
        value = self.inputs[0]
        if value < 0:

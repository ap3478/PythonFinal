...
class CalculationBase(BaseModel):
    ...
    inputs: List[float] = Field(
        ...
        example=[10.5, 3, 2],
        min_items=1  # Most ops need >= 2 — that's enforced in validate_inputs
    )
    ...
    def validate_inputs(self) -> "CalculationBase":
        ...
        Rules:
        - ``square_root`` is unary: exactly one input is required, and it must
          be non-negative.
        - All other operations require at least two inputs.
        - ``division`` and ``modulus`` reject zero in any divisor position
          (i.e., positions after the first).
        - ``power`` rejects raising 0 to a negative exponent.
        ...
        if self.type == CalculationType.POWER:
            ...
            for value in self.inputs[1:]:
                ...
                try:
                    ...
                    raise ValueError("Power operation overflowed")
        return self
...
class CalculationUpdate(BaseModel):
    ...
    inputs: Optional[List[float]] = Field(
        ...
        example=[42, 7],
        min_items=1  # Most ops need >= 2 — checked alongside type at runtime
    )
    ...
    def validate_inputs(self) -> "CalculationUpdate":
        ...
        Because the request body for an update intentionally does NOT
        include the calculation ``type`` (the type is fixed at creation),
        we only enforce the lowest common denominator here:
        a non-empty list. Type-specific rules — for example ``square_root``
        requires exactly one input, ``division`` and ``modulus`` reject
        zero divisors — are re-validated at the model level when
        ``get_result()`` runs, and the route surfaces those errors as
        HTTP 400 responses.
